...
export function createMcpRouter(sequelize: any, options: RouterOptions = {}): express.Router {
  ...
  if (options.rateLimit?.enabled ?? MCP_RATE_LIMIT_ENABLED) {
    router.use(
      RateLimit({
        windowMs: options.rateLimit?.windowMs ?? MCP_RATE_LIMIT_WINDOW_MS,
        max: options.rateLimit?.max ?? MCP_RATE_LIMIT_MAX,
        keyGenerator: (req) =>
